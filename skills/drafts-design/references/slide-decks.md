...
**Esta decisão vem antes de "arquivo único ou múltiplos arquivos".** 2026-04-20 Projeto real testado na prática: **não confirmar o formato de entrega antes de começar = 2-3 horas de retrabalho.**
...
Usuário diz "quero apresentação HTML **e** PPTX editável" — **isso não é misto**, é o requisito PPTX cobrindo o requisito HTML. O HTML escrito no formato Path A já pode ser apresentado em tela cheia no navegador (basta adicionar um `deck_index.html` agregador). **Sem custo adicional.**
...
Causa: A resolução de módulos ESM procura `node_modules` a partir do diretório do script. O script está em `~/.claude/skills/drafts-design/scripts/`, onde não há dependências.
...
| Dimensão                     | Arquivo Único + `deck_stage.js`                            | **Múltiplos Arquivos + `deck_index.html` agregador**                                    |
| ---------------------------- | ---------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| Estrutura de código          | Um HTML, todos os slides são `<section>`                   | Cada página HTML independente, `index.html` usa iframe para agregar                     |
...
| Impressão PDF                | ✅ Embutido                                                | ✅ O agregador beforeprint percorre iframes                                             |
| Navegação por teclado        | ✅ Embutido                                                | ✅ Embutido no agregador                                                                |
...
O agregador já vem com: navegação por teclado (←/→/Home/End/teclas numéricas/P para imprimir), scale + letterbox, contador no canto inferior direito, memória localStorage, navegação por hash, modo de impressão (percorre iframes e gera PDF por página).
...
**Ao verificar com `mdls -name kMDItemNumberOfPages`**, atenção: o metadata Spotlight do macOS tem cache. Após reescrever o PDF, execute `mdimport file.pdf` para forçar a atualização, caso contrário exibirá o número de páginas antigo. Use `pdfinfo` ou `pdftoppm` para contar o número real de arquivos.
