---
name: huashu-design
description: 花叔Design（Huashu-Design）—— uma skill de design completa que usa HTML para criar protótipos de alta fidelidade, Demos interativos, slides, animações, exploração de variações de design + consultor de direção de design + revisão especializada. HTML é ferramenta, não meio — incorpora diferentes especialistas conforme a tarefa (UX Designer/Animador/Slide Designer/Prototipador), evitando clichês de web design. Palavras de ativação: fazer protótipo, design Demo, protótipo interativo, demonstração HTML, Demo de animação, variação de design, design hi-fi, UI mockup, prototype, exploração de design, fazer uma página HTML, fazer uma visualização, protótipo de app, protótipo iOS, mockup de aplicativo móvel, exportar MP4, exportar GIF, vídeo 60fps, estilo de design, direção de design, filosofia de design, esquema de cores, estilo visual, recomendar estilo, escolher um estilo, fazer algo bonito, revisar, está bonito?, review this design. **Habilidades principais**: Fluxo de trabalho Junior Designer (mostrar hipóteses + reasoning + placeholders primeiro, depois iterar), Lista anti-AI slop, Melhores práticas React+Babel, Sistema Tweaks para variações, Speaker Notes, Starter Components (slide shell / canvas de variações / engine de animação / molduras de dispositivos), Regras específicas para protótipos de App (buscar imagens reais de Wikimedia/Met/Unsplash por padrão, cada iPhone com AppPhone state manager interativo, executar testes de clique com Playwright antes de entregar), Verificação Playwright, Exportação HTML animação → MP4/GIF (25fps base + 60fps interpolação + palette otimizada GIF + 6 BGMs cênicas + fade automático). **Fallback para requisitos vagos**: Modo Consultor de Direção de Design — recomendar 3 direções diferenciadas a partir de 5 escolas × 20 filosofias de design (Pentagram Arquitetura da Informação / Field.io Poética do Movimento / Kenya Hara Minimalismo Oriental / Sagmeister Vanguarda Experimental, etc.), exibir 24 showcases pré-fabricados (8 cenários × 3 estilos), gerar 3 Demos visuais em paralelo para o usuário escolher. **Pós-entrega opcional**: Revisão especializada em 5 dimensões (Consistência Filosófica / Hierarquia Visual / Execução de Detalhes / Funcionalidade / Inovação — cada uma nota 0-10 + lista de correções).
---

# Huashu-Design · Design com HTML
...
Cenários NÃO aplicáveis: Web Apps de produção, sites SEO, sistemas dinâmicos com backend — para esses, use a skill de frontend-design.
...
**Exemplo Real** (armadilha真实 em 2026-04-20):
...
5. **Fallback AI generation** (nano-banana-pro): use a imagem real do produto como referência para AI gerar variantes adequadas à cena. **Não substitua com CSS/SVG desenhado à mão**
...
>
> 2026-04-20 Palavras do Huashu: «Nosso princípio é pesquisar 5 rodadas, encontrar 10 materiais, selecionar 2 bons. Cada um precisa ter nota 8/10 ou mais. Melhor ter menos do que incluir qualquer coisa só para cumprir tarefa.»
...
- Filosofia do Huashu: **Antes falta do que sobra**. Material de baixa qualidade é pior que não ter — polui o paladar visual, transmite sinal de "não profissional"
...
| **Cores**             | `grep -hoE '#[0-9A-Fa-f]{6}' assets/<brand>-brand/\*.{svg,html,css}                                           | sort | uniq -c | sort -rn | head -20`, filtrar preto/branco/cinza |
...
**Don't add filler content**. Cada elemento precisa earn its place. Espaço em branco é problema de design, resolva com composição, não inventando conteúdo para preencher. **One thousand no's for every yes**. Especialmente cuidado com:

- «data slop» — números, ícones, estatísticas decorativas sem utilidade
- «iconography slop» — todo título com ícone
- «gradient slop» — todo fundo com gradiente
...
- ✅ `text-wrap: pretty` + CSS Grid + CSS avançado: detalhes de tipografia são o "imposto de bom gosto" que AI não sabe diferenciar — agentes que usam这些东西 parecem designers de verdade
...
| Homepage pessoal / AI导航 / AI写作 / SaaS / Documentação dev | `assets/showcases/website-*/`   |
...
**Princípio de prioridade de素材 real** (quando envolver o próprio usuário/produto):
...
| Usuário já tem素材 local        | `~/Downloads`, projeto `_archive/` ou biblioteca de素材 configurada pelo usuário          |
...
Ao fazer mockup de iPhone, **é obrigatório** usar `assets/ios_frame.jsx`. Este é o外壳 padrão com especificações precisas já alinhadas com iPhone 15 Pro: bezel, Dynamic Island (124×36, top:12, centralizado), status bar (hora/sinal/bateria,避ando a ilha em ambos os lados, vertical center alinhado com a linha central da ilha), Home Indicator, padding superior da área de conteúdo já tratados.
...
1. **Entender a necessidade**:
   ...
   - 🛑 **Tarefas de slides/PPT: a versão HTML agregada para demonstração é SEMPRE o artefato base padrão** (independente do formato final que o usuário queira):
     ...
     - **Deck ≥ 5 páginas:必须先 fazer 2 páginas showcase para definir grammar antes de produzir em lote** (veja seção "fazer showcase antes da produção em lote" em `references/slide-decks.md`) — pular isso =方向 errada, retrabalho N vezes em vez de 2
     - Veja o início de `references/slide-decks.md` — "Arquitetura HTML优先 + árvore de decisão de formato de entrega"
...
3. **Primeiro responda quatro perguntas, depois planeje o sistema**: **Esta primeira metade do passo determina a saída mais do que todas as regras CSS**.
   ...
   - **Estimativa de capacidade**: Desenhe 3 thumbnails de 5 segundos no papel para ver se o conteúdo cabe? (prevenir overflow / compressão)
     Após responder as quatro perguntas, vocalize o design system (cor/tipografia/ritmo de layout/component pattern) — **o sistema deve servir às respostas, não escolher o sistema primeiro e depois enfiar o conteúdo**.
...
5. **Junior pass**: Escreva assumptions+placeholders+reasoning comments no HTML.
...
6. **Full pass**: Preencha placeholders, faça variations, adicione Tweaks. Mostre novamente na metade, não espere terminar tudo.
...
9. **(Padrão) Exportar vídeo · Obrigatório SFX + BGM**: A **forma de entrega padrão** de HTML animado é MP4 com áudio, não apenas imagem pura. Versão sem som = produto semi-acabado — o usuário percebe subconscientemente "imagem se move mas não há resposta sonora", aí está a raiz da sensação de baixo custo. Pipeline:
   - `scripts/render-video.js` grava 25fps MP4 de imagem pura (apenas intermediário, **não é o produto final**)
...
| Imagem                   | SVG desenhando pessoas/objetos                                                                | 素材 real ou placeholder                                                                                                      |
...
- **Multi-arquivo** (padrão, ≥10 páginas / acadêmico / curso / multi-agent paralelo) → cada página HTML independente + `assets/deck_index.html`拼接器
...
| `scripts/export_deck_stage_pdf.mjs` | **HTML→PDF (arquitetura single file deck-stage)** · Novo em 2026-04-20. Lida com problemas de slot shadow DOM que geravam "só 1 página", overflow de filhos absolute, etc. Veja末節 em `references/slide-decks.md`. Dependências: `playwright`                                                                                          |
...
| **Filosofia de cena Gallery Ripple + Multi-Focus** (quando素材 20+ homogêneos+cena precisa expressar "escala×profundidade"; inclui pré-condições, receita técnica, 5 padrões reutilizáveis) | `references/hero-animation-case-study.md` (destilado do huashu-design hero v9)                                                                     |
...
**Apenas em animações** (HTML animado → MP4 / GIF), inclua por padrão a marca d'água "**Created by Huashu-Design**" para auxiliar na propagação da skill. **Slides / infográficos / protótipos / páginas web e outros cenários não incluem** — incluir atrapalharia o uso real do usuário.
...
- **Embody o especialista**: Ao fazer slides, seja um slide designer. Ao fazer animação, seja um animador. Não é escrever Web UI.
...
- **Antes de fazer animação**: Leitura obrigatória de `references/animation-pitfalls.md` — as 14 regras vieram de erros reais, pular pode custar 1-3 rodadas de retrabalho.
- **Stage / Sprite escritos à mão** (sem usar `assets/animations.jsx`): Deve implementar duas coisas — (a) no primeiro tick, sincronizar `window.__ready = true` (b) detectar `window.__recording === true` e forçar loop=false. Caso contrário, a gravação de vídeo certamente terá problemas.
